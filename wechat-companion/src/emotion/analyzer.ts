 ...
 * 更长的关键词匹配获得更高权重
 */
function calculateWeightedScore(text: string, wordList: string[]): number {
  let score = 0;
  const lowerText = text.toLowerCase();
 ...
 * 基于标点符号和文本长度调整置信度
 */
function adjustConfidenceByContext(text: string, baseConfidence: number): number {
  let adjustment = 0;
 ...
 * ```
 */
export function analyze(text: string): EmotionResult {
  ...
  for (const emotion of ['happy', 'sad', 'angry', 'anxious'] as string[]) {
  ...
  for (const [emotion, score] of Object.entries(scores) as [string, number][]) {
    if (score > topScore) {
 ...
 */
export function getToneGuidance(emotion: Emotion): string {
  const guidance: Record<Emotion, string> = {
