...
export interface UserProfile {
  ...
  /** 核心联系人 */
  keyRelationships: Array<{ name: string; relation: string }>;
  /** 情绪模式 */
 ...
 * @returns 用户画像
 */
export async function analyzeChatProfile(
...
function parseProfileResult(rawAnalysis: string, messages: ChatMessage[]): UserProfile {
  try {
    // 尝试从LLM输出中提取 JSON
...
function getMockProfile(messages: ChatMessage[]): UserProfile {
  ...
  // 情绪表情使用统计
  const emojiCount = userMsgs.filter((m) =>
    /[\u{1F300}-\u{1F9FF}]|[\u{2600}-\u{26FF}]|[\u{2700}-\u{27BF}]|[\u{1FA00}-\u{1FA6F}]/u.test(m.content)
  ).length;
 ...
 */
export function generateUserContext(profile: UserProfile): string {
  const parts: string[] = [];
