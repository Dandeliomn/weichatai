...
exports.initLongMemory = initLongMemory;
...
exports.getRecentActiveUsers = getRecentActiveUsers;
exports.upsertDailySummary = upsertDailySummary;
exports.logCareMessage = logCareMessage;
exports.hasCareMessageToday = hasCareMessageToday;
exports.getConversationStats = getConversationStats;
exports.ensureUserAccount = ensureUserAccount;
 ...
 * 初始化 PostgreSQL 连接池
 */
function initLongMemory(pgPool) {
    pool = pgPool;
 ...
 * @returns 用户记录
 */
async function getOrCreateUser(wechatId, nickname) {
    ...
    try {
       ...
       SET last_active_at = NOW(),
           ...
           updated_at = NOW()
       WHERE wechat_id = $1
       RETURNING *`, [wechatId, nickname || null]);
        if (updateResult.rows.length > 0) {
            return updateResult.rows[0];
        }
        // 用户不存在，创建新用户
        const insertResult = await client.query(`INSERT INTO users (wechat_id, nickname)
       VALUES ($1, $2)
       ...
       RETURNING *`, [wechatId, nickname || null]);
        ...
        const selectResult = await client.query(`SELECT * FROM users WHERE wechat_id = $1`, [wechatId]);
...
async function getMemories(userId, limit = 10) {
    try {
        const result = await pool.query(`SELECT * FROM user_memories
       WHERE user_id = $1
...
async function searchMemories(userId, queryText, limit = 5) {
    try {
       ...
       ORDER BY importance DESC, updated_at DESC
       LIMIT $3`, [userId, queryText, limit]);
        // 如果全文搜索无结果，回退到简单的 LIKE 匹配
        if (result.rows.length === 0) {
            const likeResult = await pool.query(`SELECT * FROM user_memories
         WHERE user_id = $1
 ...
 * @returns 创建的记忆记录
 */
async function addMemory(userId, summaryText, keywords = [], emotion = null, importance = 5, memoryType = 'factual') {
    try {
        const result = await pool.query(`INSERT INTO user_memories (user_id, summary_text, keywords, emotion, importance, memory_type)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`, [userId, summaryText, keywords, emotion, importance, memoryType]);
 ...
 * 记录对话日志 (支持媒体字段)
 */
async function logConversation(userId, wechatId, role, content, emotion, emotionConfidence, media) {
    try {
        await pool.query(`INSERT INTO conversation_logs
 ...
 * @returns 用户记录数组
 */
async function getRecentActiveUsers(days = 3) {
    try {
        const result = await pool.query(`SELECT DISTINCT u.* FROM users u
       INNER JOIN conversation_logs cl ON u.id = cl.user_id
       WHERE cl.created_at > NOW() - INTERVAL '1 day' * $1
         AND u.is_active = TRUE
       ORDER BY u.last_active_at DESC`, [days]);
        // 如果对话日志匹配的用户太少，回退到按 last_active_at 查询
        if (result.rows.length === 0) {
            const fallbackResult = await pool.query(`SELECT * FROM users
         WHERE last_active_at > NOW() - INTERVAL '1 day' * $1
           AND is_active = TRUE
         ORDER BY last_active_at DESC`, [days]);
 ...
 */
async function upsertDailySummary(userId, summaryDate, summaryText, moodSummary = null, topicKeywords = [], messageCount = 0) {
    try {
        await pool.query(`INSERT INTO daily_summaries (user_id, summary_date, summary_text, mood_summary, topic_keywords, message_count)
       VALUES ($1, $2, $3, $4, $5, $6)
       ...
       DO UPDATE SET
         ...
         message_count = daily_summaries.message_count + EXCLUDED.message_count`, [userId, summaryDate, summaryText, moodSummary, topicKeywords, messageCount]);
    }
    catch (error) {
 ...
 */
async function logCareMessage(userId, messageText, scheduleType) {
    try {
        await pool.query(`INSERT INTO care_message_logs (user_id, message_text, schedule_type)
       VALUES ($1, $2, $3)`, [userId, messageText, scheduleType]);
    }
    catch (error) {
 ...
 * @returns 是否已发送
 */
async function hasCareMessageToday(userId, scheduleType) {
    try {
        const result = await pool.query(`SELECT 1 FROM care_message_logs
       WHERE user_id = $1
         AND schedule_type = $2
         AND sent_at::date = CURRENT_DATE
       LIMIT 1`, [userId, scheduleType]);
 ...
 * @returns 统计信息
 */
async function getConversationStats(userId, since) {
    try {
 ...
 */
async function ensureUserAccount(wechatId, nickname, msgId) {
    try {
        const existing = await pool.query('SELECT id, email FROM user_accounts WHERE wechat_id = $1', [wechatId]);
        ...
        for (let attempt = 0; attempt < 5; attempt++) {
            ...
            const result = await pool.query(`INSERT INTO user_accounts (email, password_hash, display_name, wechat_id, role)
         VALUES ($1,$2,$3,$4,'user') ON CONFLICT (email) DO NOTHING RETURNING id`, [tryEmail, hash, nickname || wechatId, wechatId]);
            if (result.rows.length > 0) {
        ...
        console.error('[LongMemory] 创建账号失败:', error);
        return null;
    }
}
async function changePassword(wechatId, newPassword) {
    try {
        const bcrypt = require('bcryptjs');
        const hash = await bcrypt.hash(newPassword, 10);
        const r = await pool.query('UPDATE user_accounts SET password_hash=$1, updated_at=NOW() WHERE wechat_id=$2', [hash, wechatId]);
        return (r.rowCount || 0) > 0;
    }
    catch (error) {
        console.error('[LongMemory] 修改密码失败:', error);
        return false;
    }
}
