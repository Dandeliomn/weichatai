...
exports.extractStructuredMemories = extractStructuredMemories;
 ...
 * 从采样消息中提取结构化记忆
 */
async function extractFromBatch(messages, aiName, userName, category) {
...
每条 summary 必须是独立、完整的中文句子。从对话中提取真实信息，不要虚构。`;
    try {
        ...
        if (jsonMatch) {
            ...
            return items.filter((item) => item.summary &&
                item.summary.length >= 5 &&
                Array.isArray(item.keywords) &&
                item.keywords.length > 0).map((item) => ({
 ...
 * 从导入任务中采样消息
 */
async function sampleMessages(pgPool, limit) {
    const task = await pgPool.query(`SELECT id, meta FROM import_tasks WHERE status = 'done' ORDER BY created_at DESC LIMIT 1`);
    if (task.rows.length === 0)
        return { messages: [], total: 0 };
    const taskId = task.rows[0].id;
    const meta = task.rows[0].meta || {};
    const aiName = meta.aiName || 'AI';
    const count = await pgPool.query(`SELECT COUNT(*) as cnt FROM imported_messages WHERE task_id = $1`, [taskId]);
    ...
    for (let i = 0; i < Math.min(limit, total); i++) {
        ...
        const row = await pgPool.query(`SELECT sender, content, timestamp FROM imported_messages
       WHERE task_id = $1 AND length(content) > 5
       ORDER BY timestamp ASC OFFSET $2 LIMIT 1`, [taskId, offset]);
 ...
 * @returns 提取的记忆数量
 */
async function extractStructuredMemories(pgPool) {
    ...
    // 读取导入任务的 meta 获取角色名
    const taskMeta = await pgPool.query(`SELECT id, user_id, meta FROM import_tasks WHERE status = 'done' ORDER BY created_at DESC LIMIT 1`);
    ...
    const userInfo = await pgPool.query(`SELECT id FROM users WHERE wechat_id ILIKE '%@im.wechat%' LIMIT 1`);
    ...
    for (let i = 0; i < batchCount; i++) {
        ...
        for (const mem of memories) {
            try {
                // 检查是否已有相似记忆
                const existing = await pgPool.query(`SELECT id FROM user_memories
           WHERE user_id = $1 AND summary_text ILIKE $2 LIMIT 1`, [memoryUserId, `%${mem.summary.substring(0, 30)}%`]);
                if (existing.rows.length > 0) {
                    // 更新已有记忆（合并关键词）
                    await pgPool.query(`UPDATE user_memories
             SET keywords = array_cat(keywords, $3::text[]),
                 importance = GREATEST(importance, $4),
                 updated_at = NOW()
             WHERE id = $1`, [existing.rows[0].id, mem.keywords, mem.importance]);
                }
                else {
                    await pgPool.query(`INSERT INTO user_memories (user_id, summary_text, keywords, emotion, importance, memory_type)
             VALUES ($1, $2, $3, 'neutral', $4, 'factual')`, [memoryUserId, mem.summary, mem.keywords, mem.importance]);
                }
                totalExtracted++;
            }
            catch (e) {
