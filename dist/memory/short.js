...
exports.initShortMemory = initShortMemory;
 ...
 * 初始化 Redis 连接
 */
function initShortMemory(redisInstance) {
    redis = redisInstance;
 ...
 */
async function addMessage(sessionId, role, content, emotion) {
    if (!redis) {
 ...
 * @returns 记忆条目数量
 */
async function getMemoryCount(sessionId) {
    if (!redis)
        return 0;
    try {
