...
exports.initImportRoutes = initImportRoutes;
...
let pgPool;
let importQueue;
function initImportRoutes(pool, redis) {
    pgPool = pool;
    importQueue = new bullmq_1.Queue('chat-import', { connection: redis });
}
// 所有路由需要认证
...
router.get('/tasks', async (req, res) => {
    try {
        const result = await pgPool.query('SELECT id, filename, status, message_count, created_at FROM import_tasks WHERE user_id=$1 ORDER BY created_at DESC LIMIT 20', [req.user.userId]);
...
router.post('/presign', async (req, res) => {
    try {
        ...
        // 创建导入任务记录
        const taskResult = await pgPool.query(`INSERT INTO import_tasks (user_id, filename, file_path, file_size, format, status)
       VALUES ($1, $2, $3, $4, 'auto', 'pending')
       RETURNING id`, [req.user.userId, filename, `minio://${bucket}/${filename}`, fileSize || 0]);
...
router.post('/confirm', async (req, res) => {
    try {
        ...
        if (endPoint && accessKey && secretKey) {
            ...
            // 更新任务 + 入队处理
            await pgPool.query(`UPDATE import_tasks SET file_path=$1, file_size=$2, status='pending' WHERE id=$3`, [localPath, fileSize, taskId]);
...
// =============================================================================
router.post('/upload', upload_1.handleUpload, async (req, res) => {
    try {
        const file = req.file;
        if (!file) {
        ...
        // 创建导入任务
        const result = await pgPool.query(`INSERT INTO import_tasks (user_id, filename, file_path, file_size, format, status)
       ...
       RETURNING id`, [
        ...
        if (isArchive) {
            const extractDir = path_1.default.join(path_1.default.dirname(file.path), 'extracted_' + taskId);
            ...
            try {
                ...
                function walkDir(dir) {
                    for (const entry of fs_1.default.readdirSync(dir, { withFileTypes: true })) {
                        ...
                        if (entry.isDirectory()) {
                            ...
                            const e = path_1.default.extname(entry.name).toLowerCase();
                            if (chatExts.includes(e))
                                chatFiles.push(full);
                            if (stickerExts.includes(e))
                                stickerList.push(full);
                        }
                    }
                }
                walkDir(extractDir);
                ...
                if (chatFiles.length > 0) {
                    const safeExt = path_1.default.extname(chatFiles[0]);
                    const safePath = path_1.default.join(extractDir, `chat_${taskId}${safeExt}`);
                    try {
                        fs_1.default.copyFileSync(chatFiles[0], safePath);
                    }
                    catch {
                        const fallback = findFileByExt(extractDir, safeExt);
                        if (fallback) {
                            fs_1.default.renameSync(fallback, safePath);
                        }
                    }
                    finalPath = safePath;
        ...
        if (nonArchiveStickerExts.includes(ext)) {
            ...
            try {
                fs_1.default.mkdirSync(userStickerDir, { recursive: true, mode: 0o777 });
            }
            catch { }
            fs_1.default.copyFileSync(file.path, path_1.default.join(userStickerDir, file.originalname));
            stickerCount = 1;
        }
        const metaPayload = { chatFiles: chatFileCount, stickers: stickerCount, isArchive };
        await pgPool.query(`UPDATE import_tasks SET meta=$1::jsonb WHERE id=$2`, [JSON.stringify(metaPayload), taskId]);
...
// =============================================================================
router.post('/weflow-api', async (req, res) => {
    try {
        ...
        const fileDir = path_1.default.join(process.env.UPLOAD_DIR || 'uploads', 'weflow-api');
        fs_1.default.mkdirSync(fileDir, { recursive: true });
        const filePath = path_1.default.join(fileDir, filename);
        fs_1.default.writeFileSync(filePath, JSON.stringify(bundle));
        // 获取消息统计
        const messageCount = bundle.messages.length;
        const stickerCount = bundle.messages.filter((m) => ['sticker', 'emoji'].includes(m.msgType)).length;
        const imageCount = bundle.messages.filter((m) => m.msgType === 'image').length;
        console.log(`[Import-WeFlow] 收到数据: ${messageCount}条消息 ` +
            `(表情:${stickerCount}, 图片:${imageCount}, ` +
            `联系人:${bundle.contact?.name || '未知'})`);
        // 创建导入任务
        const result = await pgPool.query(`INSERT INTO import_tasks (user_id, filename, file_path, file_size, format, status, message_count)
       VALUES ($1, $2, $3, $4, 'weflow-api', 'pending', $5)
       RETURNING id`, [
            req.user.userId,
            `WeFlow API: ${bundle.contact?.name || '未知'}`,
            filePath,
            fs_1.default.statSync(filePath).size,
            messageCount,
        ]);
        const taskId = result.rows[0].id;
        // 加入处理队列
...
router.get('/status/:id', async (req, res) => {
    try {
        ...
        const result = await pgPool.query(`SELECT * FROM import_tasks WHERE id = $1 AND user_id = $2`, [taskId, req.user.userId]);
        ...
        let messageSample = [];
        if (task.status === 'done') {
            const messages = await pgPool.query(`SELECT sender, content, timestamp, msg_type
...
router.get('/analysis/:id', async (req, res) => {
    try {
        const taskId = parseInt(req.params.id);
        const task = await pgPool.query(`SELECT * FROM import_tasks WHERE id = $1 AND user_id = $2`, [taskId, req.user.userId]);
        ...
        // 获取发送者统计
        const senderStats = await pgPool.query(`SELECT sender, COUNT(*) as count,
       ...
       WHERE task_id = $1
       GROUP BY sender
       ORDER BY count DESC`, [taskId]);
        // 获取时间分布
        const timeDistribution = await pgPool.query(`SELECT EXTRACT(HOUR FROM timestamp)::int as hour, COUNT(*) as count
       FROM imported_messages
       WHERE task_id = $1
       GROUP BY hour
       ORDER BY hour`, [taskId]);
        ...
        try {
            const sample = await pgPool.query(`SELECT sender, content FROM imported_messages WHERE task_id=$1 ORDER BY RANDOM() LIMIT 200`, [taskId]);
            const conversation = sample.rows.map((r) => `${r.sender}: ${r.content}`).join('\n');
            senders = [...new Set(sample.rows.map((r) => r.sender))];
...
router.post('/apply', async (req, res) => {
    try {
        ...
        const task = await pgPool.query(`SELECT * FROM import_tasks WHERE id = $1 AND user_id = $2 AND status = 'done'`, [taskId, req.user.userId]);
        if (task.rows.length === 0) {
...
router.get('/tasks', async (req, res) => {
    try {
        ...
        const [tasks, countResult] = await Promise.all([
            pgPool.query(`SELECT id, filename, format, status, message_count,
         ...
         LIMIT $2 OFFSET $3`, [req.user.userId, limit, offset]),
            pgPool.query('SELECT COUNT(*) FROM import_tasks WHERE user_id = $1', [req.user.userId]),
        ]);
        res.json({
            tasks: tasks.rows,
