...
router.get('/users', async (req, res) => {
    try {
        ...
        const [users, countResult] = await Promise.all([
            pgPool.query(`SELECT id, email, display_name, role, wechat_id, is_active, last_login_at, created_at
         FROM user_accounts ${whereClause}
         ORDER BY created_at DESC LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`, [...params, limit, offset]),
            pgPool.query(`SELECT COUNT(*) FROM user_accounts ${whereClause}`, params),
        ]);
        // 记录操作日志
        await pgPool.query(`INSERT INTO admin_logs (admin_id, action, target_type, details, ip_address)
       VALUES ($1, 'list_users', 'user', $2, $3)`, [req.user.userId, JSON.stringify({ search, role, page }), req.ip]);
        res.json({
            users: users.rows,
...
router.get('/users/:id', async (req, res) => {
    try {
        ...
        const [user, profile, sessions, imports] = await Promise.all([
            pgPool.query(`SELECT id, email, display_name, role, wechat_id, avatar_url, is_active, last_login_at, created_at
         FROM user_accounts WHERE id = $1`, [userId]),
            pgPool.query('SELECT * FROM user_profiles WHERE user_id = $1', [userId]),
            pgPool.query(`SELECT COUNT(*) FROM user_sessions WHERE user_id = $1 AND is_revoked = FALSE`, [userId]),
            pgPool.query(`SELECT id, filename, status, message_count, created_at FROM import_tasks WHERE user_id = $1 ORDER BY created_at DESC LIMIT 10`, [userId]),
        ]);
...
router.put('/users/:id', async (req, res) => {
    try {
        ...
        params.push(userId);
        const result = await pgPool.query(`UPDATE user_accounts SET ${updates.join(', ')} WHERE id = $${paramIndex}
       RETURNING id, email, display_name, role, is_active`, params);
        if (result.rows.length === 0) {
            res.status(404).json({ error: '用户不存在' });
            return;
        }
        // 记录日志
        await pgPool.query(`INSERT INTO admin_logs (admin_id, action, target_type, target_id, details, ip_address)
       VALUES ($1, 'update_user', 'user', $2, $3, $4)`, [req.user.userId, userId, JSON.stringify(req.body), req.ip]);
...
router.delete('/users/:id', async (req, res) => {
    try {
        ...
        const result = await pgPool.query('DELETE FROM user_accounts WHERE id = $1 RETURNING id, email', [userId]);
        if (result.rows.length === 0) {
            res.status(404).json({ error: '用户不存在' });
            return;
        }
        await pgPool.query(`INSERT INTO admin_logs (admin_id, action, target_type, target_id, details, ip_address)
       VALUES ($1, 'delete_user', 'user', $2, $3, $4)`, [req.user.userId, userId, JSON.stringify({ email: result.rows[0].email }), req.ip]);
...
router.get('/stats', async (_req, res) => {
    try {
        const [accountStats, wechatUserStats, messageStats, emotionStats, importStats, activeStats,] = await Promise.all([
            pgPool.query(`SELECT
           COUNT(*) as total,
         ...
         FROM user_accounts`),
            pgPool.query(`SELECT COUNT(*) as total, SUM(total_messages) as total_messages
         FROM users WHERE is_active = TRUE`),
            pgPool.query(`SELECT COUNT(*) as total,
                COUNT(*) FILTER (WHERE created_at > NOW() - INTERVAL '24 hours') as today
         FROM conversation_logs`),
            pgPool.query(`SELECT emotion, COUNT(*) as count
         ...
         FROM import_tasks`),
            pgPool.query(`SELECT COUNT(*) as today_active
...
// =============================================================================
router.get('/care-templates', async (req, res) => {
    try {
...
// =============================================================================
router.put('/care-templates', async (req, res) => {
    try {
        ...
        for (const t of templates) {
            if (t.id) {
                await pgPool.query(`UPDATE care_templates
           SET content = COALESCE($1, content),
               schedule_type = COALESCE($2, schedule_type),
               is_active = COALESCE($3, is_active),
               sort_order = COALESCE($4, sort_order)
           WHERE id = $5`, [t.content, t.scheduleType, t.isActive, t.sortOrder, t.id]);
            }
            else {
                await pgPool.query(`INSERT INTO care_templates (schedule_type, content, sort_order, created_by)
           VALUES ($1, $2, $3, $4)`, [t.scheduleType || 'morning', t.content, t.sortOrder || 0, req.user.userId]);
            }
        }
        await pgPool.query(`INSERT INTO admin_logs (admin_id, action, target_type, details, ip_address)
       VALUES ($1, 'update_care_templates', 'template', $2, $3)`, [req.user.userId, JSON.stringify({ count: templates.length }), req.ip]);
...
router.get('/logs', async (req, res) => {
    try {
        ...
        const [logs, countResult] = await Promise.all([
            pgPool.query(`SELECT al.*, ua.email as admin_email
         FROM admin_logs al
...
router.get('/conversations', async (req, res) => {
    try {
        ...
        const limit = Math.min(parseInt(req.query.limit) || 20, 100);
        const offset = (page - 1) * limit;
        const wechatId = req.query.wechat_id || '';
        ...
        let idx = 1;
        if (wechatId) {
            conditions.push(`cl.wechat_id ILIKE $${idx}`);
            params.push(`%${wechatId}%`);
            idx++;
        }
        if (keyword) {
            conditions.push(`cl.content ILIKE $${idx}`);
            params.push(`%${keyword}%`);
            idx++;
        }
        if (emotion && ['happy', 'sad', 'angry', 'anxious', 'neutral'].includes(emotion)) {
            conditions.push(`cl.emotion = $${idx}`);
            params.push(emotion);
            idx++;
        }
        if (dateFrom) {
            conditions.push(`cl.created_at >= $${idx}::timestamp`);
        ...
        res.status(500).json({ error: '获取对话日志失败' });
    }
});
// =============================================================================
...
router.get('/users/:id/emotions', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: '获取情绪分布失败' });
    }
});
// =============================================================================
...
// =============================================================================
router.get('/users/:id/memories', async (req, res) => {
    try {
...
// =============================================================================
router.get('/users/:id/summaries', async (req, res) => {
    try {
...
router.get('/invite-codes', async (_req, res) => {
    try {
        const result = await pgPool.query(`SELECT ic.*, ua.email as creator_email FROM invite_codes ic
...
// =============================================================================
router.post('/recharge-codes', async (req, res) => {
    try {
...
// =============================================================================
router.get('/recharge-codes', async (_req, res) => {
    try {
        const result = await pgPool.query(`SELECT rc.*, ua.email as creator_email, ub.email as used_by_email
...
router.put('/users/:id/membership', async (req, res) => {
    try {
        ...
        await pgPool.query('UPDATE user_accounts SET membership=$1 WHERE id=$2', [tier, parseInt(req.params.id)]);
...
router.put('/config', async (req, res) => {
    try {
        ...
        await pgPool.query('INSERT INTO system_config (key, value) VALUES ($1,$2) ON CONFLICT (key) DO UPDATE SET value=$2, updated_at=NOW()', [key, value]);
...
router.get('/dashboard', async (_req, res) => {
    try {
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const row = trend.rows.find((r) => r.summary_date === ds);
            trendData.push({ date: ds, messages: row ? parseInt(row.message_count) || 0 : 0 });
        }
