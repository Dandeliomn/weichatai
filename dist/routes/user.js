...
router.get('/profile', async (req, res) => {
    try {
        const user = await pgPool.query(`SELECT id, email, display_name, role, wechat_id, avatar_url, is_active,
       ...
       FROM user_accounts WHERE id = $1`, [req.user.userId]);
        ...
        if (user.rows[0]?.wechat_id) {
            const stats = await pgPool.query(`SELECT total_messages, first_active_at, last_active_at
         FROM users WHERE wechat_id = $1`, [user.rows[0].wechat_id]);
...
router.put('/profile', async (req, res) => {
    try {
        ...
        const result = await pgPool.query(`UPDATE user_accounts
       SET display_name = COALESCE($1, display_name),
           avatar_url = COALESCE($2, avatar_url)
       WHERE id = $3
       RETURNING id, email, display_name, avatar_url`, [displayName || null, avatarUrl || null, req.user.userId]);
...
router.get('/conversations', async (req, res) => {
    try {
        ...
        // 需要先获取关联的 wechat_id
        const user = await pgPool.query('SELECT wechat_id FROM user_accounts WHERE id = $1', [req.user.userId]);
        if (!user.rows[0]?.wechat_id) {
        ...
        const [convResult, countResult] = await Promise.all([
            pgPool.query(`SELECT id, role, content, emotion, emotion_confidence, created_at
         FROM conversation_logs
         WHERE wechat_id = $1
         ORDER BY created_at DESC
         LIMIT $2 OFFSET $3`, [wechatId, limit, offset]),
            pgPool.query(`SELECT COUNT(*) FROM conversation_logs WHERE wechat_id = $1`, [wechatId]),
        ]);
        res.json({
...
router.get('/memories', async (req, res) => {
    try {
        ...
        // 找到 users 表中的记录
        const wechatUser = await pgPool.query('SELECT id FROM users WHERE wechat_id = $1', [user.rows[0].wechat_id]);
        if (wechatUser.rows.length === 0) {
            res.json({ memories: [] });
            return;
        }
        const memories = await pgPool.query(`SELECT id, summary_text, keywords, emotion, importance, memory_type, created_at
       FROM user_memories
       WHERE user_id = $1
       ORDER BY importance DESC, updated_at DESC
       LIMIT 50`, [wechatUser.rows[0].id]);
...
router.get('/stats', async (req, res) => {
    try {
        ...
        const stats = await pgPool.query(`SELECT
         COUNT(*) as total_messages,
         COUNT(*) FILTER (WHERE role = 'user') as user_messages,
         COUNT(*) FILTER (WHERE role = 'assistant') as assistant_messages,
         COUNT(DISTINCT DATE(created_at)) as active_days
       FROM conversation_logs
       WHERE wechat_id = $1`, [user.rows[0].wechat_id]);
        // 情绪分布
        const emotions = await pgPool.query(`SELECT emotion, COUNT(*) as count
       FROM conversation_logs
       WHERE wechat_id = $1 AND emotion IS NOT NULL
       GROUP BY emotion
       ORDER BY count DESC`, [user.rows[0].wechat_id]);
        // 最近7天趋势
        const trend = await pgPool.query(`SELECT DATE(created_at) as date, COUNT(*) as count
       FROM conversation_logs
       WHERE wechat_id = $1 AND created_at > NOW() - INTERVAL '7 days'
       GROUP BY DATE(created_at)
       ORDER BY date`, [user.rows[0].wechat_id]);
        res.json({
            stats: stats.rows[0],
...
// =============================================================================
router.put('/ai-prefs', async (req, res) => {
    try {
        const { personality, communicationStyle, hobbies, customPrompt, modelPrefs } = req.body;
        const result = await pgPool.query(`INSERT INTO user_profiles (user_id, personality, communication_style, hobbies, custom_prompt, model_prefs)
       VALUES ($1, $2, $3, $4, $5, $6)
       ON CONFLICT (user_id)
       DO UPDATE SET
...
router.get('/invite-codes', async (req, res) => {
    try {
        const result = await pgPool.query('SELECT * FROM invite_codes WHERE created_by=$1 ORDER BY created_at DESC LIMIT 50', [req.user.userId]);
...
router.post('/redeem', async (req, res) => {
    try {
        ...
        if (r.rows.length === 0) {
            res.status(400).json({ error: '充值码无效或已使用' });
            return;
        }
        const rc = r.rows[0];
...
const stickerUpload = (0, multer_1.default)({
    storage: multer_1.default.diskStorage({
        destination: (_req, _f, cb) => {
            const dir = `/app/stickers/${_req.user?.userId || 'default'}`;
        ...
        for (const f of files) {
            if (f.originalname.endsWith('.zip')) {
                try {
                    ...
                    const extracted = require('fs').readdirSync(dir).filter((x) => /\.(gif|png|jpg|jpeg|webp)$/i.test(x));
                    results.push(...extracted.map((x) => `/stickers/${req.user.userId}/${x}`));
                    require('fs').unlinkSync(f.path);
                }
                catch (e) {
                    results.push(`zip提取失败: ${e.message}`);
                }
            }
            else {
...
router.get('/stickers', (req, res) => {
    try {
        ...
        const files = require('fs').readdirSync(dir).filter((x) => /\.(gif|png|jpg|jpeg|webp)$/i.test(x));
        res.json({ stickers: files.map((f) => `/stickers/${req.user.userId}/${f}`) });
    }
    catch {
        res.json({ stickers: [] });
    }
...
// =============================================================================
router.delete('/stickers', (req, res) => {
    try {
        const dir = `/app/stickers/${req.user.userId}`;
        const fs = require('fs');
