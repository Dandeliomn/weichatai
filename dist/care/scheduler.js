 ...
 * 获取指定时段的关怀文案
 */
function getMessagesByScheduleType(scheduleType) {
    switch (scheduleType) {
        case 'morning':
 ...
 * 执行主动关怀任务
 */
async function sendCareMessages() {
    ...
    try {
        ...
        // 1. 查询最近活跃用户
        // -------------------------------------------------------------------------
        const activeUsers = await (0, long_1.getRecentActiveUsers)(RECENT_DAYS);
        ...
        for (const user of activeUsers) {
            try {
                // 检查是否已发送过该时段的关怀
                const alreadySent = await (0, long_1.hasCareMessageToday)(user.id, scheduleType);
                if (alreadySent) {
                ...
                // 记录发送日志
                await (0, long_1.logCareMessage)(user.id, personalizedMessage, scheduleType);
                sentCount++;
...
async function start() {
    ...
    console.log('');
    // 注册定时任务
    const task = node_cron_1.default.schedule(CRON_SCHEDULE, () => {
 ...
 * 获取下次触发的友好描述
 */
function getNextTriggerDescription() {
    const now = new Date();
