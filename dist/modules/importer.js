...
exports.parseChatExport = parseChatExport;
exports.parseChatExportStreaming = parseChatExportStreaming;
 ...
 * @returns 解析出的消息数组
 */
async function parseChatExport(filePath, format = 'auto', options = {}) {
 ...
 */
async function parseChatExportStreaming(filePath, format, options, onBatch) {
    ...
    const flushBatch = async () => {
        ...
        batch = [];
        // 提示 GC (建议而非强制)
        if (totalMessages % 5000 === 0 && global.gc) {
            global.gc();
        }
    };
    // JSONL 流式读取 (逐行 parse)
    if (detectedFormat === 'json') {
        const rl = (0, readline_1.createInterface)({ input: (0, fs_2.createReadStream)(filePath, { highWaterMark: 64 * 1024 }), crlfDelay: Infinity });
 ...
 * t: 时间戳 (秒)
 */
function parseWeFlowHTML(data, options, html) {
    const messages = [];
    let seqId = 0;
...
function parseJSON(json, options) {
    ...
    const messageList = data.messages || data.chatLog || data.conversations || data.data || data;
    const items = Array.isArray(messageList) ? messageList : [messageList];
    return items
        .map((item, index) => convertJSONObject(item, options, index))
        .filter(Boolean);
}
function convertJSONObject(obj, options, index = 0) {
    if (!obj || typeof obj !== 'object')
        return null;
    const sender = obj.sender || obj.speaker || obj.talker || obj.name || obj.from || '未知';
    ...
    return {
        ...
        msgType: msgType === 'text' ? 'text' : 'other',
        isFromUser: isUserSender(sender, options),
        seqId: index,
    };
}
 ...
 */
function parseWeFlowAPIBundle(json, options) {
    let bundle;
    try {
    ...
    if (bundle.exporter === 'weflow-api' && Array.isArray(bundle.messages)) {
        ...
        return bundle.messages.map((m, idx) => convertWeFlowAPIMessage(m, idx, options));
    }
    // 也可能是 ChatLab 格式
    if (bundle.format === 'chatlab' || bundle.chatlab) {
        ...
        return messages.map((m, idx) => convertWeFlowAPIMessage(m, idx, options));
    }
    // 直接消息数组
    if (Array.isArray(bundle)) {
        console.log(`[Importer] 原始消息数组: ${bundle.length} 条`);
        return bundle.map((m, idx) => convertWeFlowAPIMessage(m, idx, options));
    }
 ...
 * 转换单条 WeFlow API 消息
 */
function convertWeFlowAPIMessage(m, idx, options) {
    // 消息类型映射
    const msgTypeMap = {
        text: 'text',
        image: 'image',
...
function parseChineseTime(timeStr) {
    ...
    if (isNaN(date.getTime())) {
        ...
        if (cnMatch) {
            return new Date(parseInt(cnMatch[1]), parseInt(cnMatch[2]) - 1, parseInt(cnMatch[3]), parseInt(cnMatch[4]), parseInt(cnMatch[5]));
        }
        return new Date();
    }
    return date;
 ...
 * 判断是否为用户本人的消息
 */
function isUserSender(sender, options) {
    if (!options.userNickname && !options.userWechatId)
        return false;
    const senderLower = sender.toLowerCase().trim();
    if (options.userNickname) {
        if (senderLower === options.userNickname.toLowerCase().trim())
            return true;
        if (senderLower.includes(options.userNickname.toLowerCase().trim()))
            return true;
    }
    if (options.userWechatId) {
        if (senderLower === options.userWechatId.toLowerCase().trim())
            return true;
    }
    return false;
}
...
// 后处理
// =============================================================================
function postProcess(messages, options) {
    // 1. 按时间排序
    ...
    // 3. 过滤系统消息和空消息
    const filtered = deduped.filter((m) => m.content.trim().length > 0 &&
        m.msgType !== 'system' &&
 ...
 * 返回出现次数最多的带引号名字。
 */
function extractNameFromSystemMessages(messages, defaultName) {
    const nameCounts = {};
