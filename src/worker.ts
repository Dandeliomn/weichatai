...
import { initShortMemory, getContext, addMessage } from './memory/short';
import {
  initLongMemory,
  getOrCreateUser,
  getMemories,
...
import { analyze, getToneGuidance, getEmotionDescription } from './emotion/analyzer';
...
async function loadActiveCharacter(
  ...
  try {
    ...
    const result = await pgPool.query(
      `SELECT uc.*, ct.name, ct.personality, ct.scenario,
    ...
    if (result.rows.length === 0) {
      ...
      if (fallback.rows.length > 0) return buildCharacterPrompt(fallback.rows[0]);
    }

    // 最后查管理员(role=admin)的角色，让管理员后台设的角色在微信上也生效
    if (result.rows.length === 0) {
      const adminChar = await pgPool.query(
        `SELECT uc.*, ct.name, ct.personality, ct.scenario,
                ct.first_message, ct.example_dialogue,
                ct.system_prompt, ct.post_history, ct.description
         FROM user_characters uc
      ...
      if (adminChar.rows.length > 0) return buildCharacterPrompt(adminChar.rows[0]);
    }
 ...
 * 从角色数据构建系统Prompt
 */
function buildCharacterPrompt(row: any): string | null {
  if (row.custom_prompt) return row.custom_prompt;
  const name = row.name;
  if (!name) return null;
...
function hasSystemPrompt(row: any): boolean {
 ...
 * 当用户今日对话达到一定数量时触发
 */
async function maybeGenerateDailySummary(userId: number): Promise<void> {
  try {
    ...
    if (stats.totalMessages >= 10) {
      ...
      await upsertDailySummary(
        ...
        today,
        summaryText,
        stats.emotions ? Object.keys(stats.emotions)[0] : null,
        [],
        stats.totalMessages
      );
  ...
  try {
    ...
    if (keywords.length > 0) {
      ...
      await addMemory(
        ...
        keywords,
        emotion,
        keywords.length > 3 ? 7 : 5, // 关键词多 → 重要度高
        'factual'
      );
    }
  } catch (error) {
 ...
 * 处理单条微信消息
 */
async function processMessage(job: Job<ProcessMessageJob>): Promise<string> {
  ...
  console.log(
    `[Worker] 🔄 处理消息 (jobId=${job.id}, sessionId=${sessionId}, ` +
  ...
  // 1. 获取或创建用户
  // ---------------------------------------------------------------------------
  const user = await getOrCreateUser(wechatId);

  // 自动创建仪表盘登录账号 + 获取 account ID
  const account = await ensureUserAccount(wechatId, user.nickname || undefined, job.data.msgId);
  ...
  if (isText) {
    ...
    if (pwdMatch) {
      const ok = await changePassword(wechatId, pwdMatch[2].trim());
      ...
      await logConversation(user.id, wechatId, 'user', content);
      await logConversation(user.id, wechatId, 'assistant', reply);
      if (weclawClient.isConfigured()) await weclawClient.sendMessage(reply, botId, sessionId);
      return reply;
    }
    // 角色列表
    if (content.trim() === '角色列表') {
      const chars = await pgPool.query('SELECT id, name, tagline FROM character_templates ORDER BY id');
      const list = chars.rows.map((c: any) => `${c.id}. ${c.name} — ${c.tagline}`).join('\n');
      const reply = `🎭 可选角色:\n${list}\n\n发送"使用角色 <编号>"切换`;
      await addMessage(sessionId, 'user', content);
      await addMessage(sessionId, 'assistant', reply);
      await logConversation(user.id, wechatId, 'user', content);
    ...
    if (charMatch) {
      ...
      await pgPool.query(
        `INSERT INTO user_characters (user_id, template_id, linked_wechat_id, is_active)
         VALUES ($1,$2,$3,TRUE) ON CONFLICT (user_id,template_id,linked_wechat_id) DO UPDATE SET is_active=TRUE`,
        [acctId, c.id, wechatId]
      );
    ...
    if (createMatch) {
      ...
      const result = await pgPool.query(
        `INSERT INTO character_templates (name, tagline, description, personality, category, tags, creator_id, is_official)
         VALUES ($1,$2,$3,$4,'custom',ARRAY['自定义'],$5,FALSE) RETURNING id`,
        [name, desc.substring(0, 100), desc, desc, acctId]
      );
      // 自动激活
      const cid = result.rows[0].id;
      await pgPool.query(`UPDATE user_characters SET is_active=FALSE WHERE user_id=$1 AND linked_wechat_id=$2`, [acctId, wechatId]);
      await pgPool.query(
        `INSERT INTO user_characters (user_id,template_id,linked_wechat_id,is_active) VALUES ($1,$2,$3,TRUE)
         ON CONFLICT (user_id,template_id,linked_wechat_id) DO UPDATE SET is_active=TRUE`,
        [user.id, cid, wechatId]
      );
    ...
    if (content.trim() === '分析性格') {
      const logs = await pgPool.query(
        `SELECT content FROM conversation_logs WHERE user_id=$1 AND role='user' ORDER BY created_at DESC LIMIT 50`,
        [user.id]
      );
      ...
      const sample = logs.rows.map((r: any) => r.content).join('\n');
      const analysisPrompt = `根据以下聊天记录，分析说话者的性格特点，输出一段200字以内的角色设定（含性格、说话风格、价值观），用于AI角色扮演：\n\n${sample.substring(0, 3000)}`;
      try {
        const resp = await axios.post(`${DEEPSEEK_BASE_URL}/v1/chat/completions`, {
  ...
  if (!isText && mediaType) {
    ...
    // 存储短期记忆 (用描述代替实际媒体)
    await addMessage(sessionId, 'user', displayContent, emotion || 'neutral');
    await addMessage(sessionId, 'assistant', reply);

    // 记录日志 (含媒体信息)
    await logConversation(user.id, wechatId, 'user', displayContent, emotion, emotionConfidence, {
      mediaType,
  ...
  // 2. 读取短期上下文 (最近10轮对话)
  // ---------------------------------------------------------------------------
  const shortTermMemories = await getContext(sessionId);
  ...
  try {
    ...
    if (searchTerm.length >= 4) {
      const related = await pgPool.query(
        `SELECT sender, content FROM imported_messages
  ...
  // 4. 读取长期记忆 (AI 自动生成的摘要)
  // ---------------------------------------------------------------------------
  const longTermMemories = await getMemories(user.id, 10);

  // 同时搜索与当前消息相关的记忆
  const relevantMemories = await searchMemories(user.id, content, 5);

  // 合并去重
  const allMemories = [...longTermMemories];
  for (const mem of relevantMemories) {
    if (!allMemories.find((m: any) => m.id === mem.id)) {
  ...
  const longTermMemoryText = allMemories
    .slice(0, 8)
    .map((m: any) => `- ${m.summary_text}`)
    .join('\n');

  // ---------------------------------------------------------------------------
  // 4. 情绪分析 (使用传入的预分析结果，Worker中也重新分析一次)
  // ---------------------------------------------------------------------------
  const freshEmotion = analyze(content);
  const finalEmotion = freshEmotion.confidence > 0.5 ? freshEmotion.emotion : emotion;
  const toneGuidance = getToneGuidance(finalEmotion as any);
  ...
  // ---------------------------------------------------------------------------
  await addMessage(sessionId, 'user', content, finalEmotion);
  await addMessage(sessionId, 'assistant', reply);
  ...
  // ---------------------------------------------------------------------------
  // 记录对话日志 (含媒体信息)
  await logConversation(user.id, wechatId, 'user', content, finalEmotion, freshEmotion.confidence, {
    mediaType: mediaType || '',
    mediaUrl: mediaUrl || null,
    mediaData: null,
    mediaMime: mediaMime || null,
  ...
  // 提取并存储长期记忆 (异步，不阻塞回复)
  extractAndStoreMemory(user.id, content, reply, finalEmotion).catch((err) =>
    console.error('[Worker] 异步存储记忆失败:', err)
  );

  // 检查是否需要生成每日摘要 (异步)
  maybeGenerateDailySummary(user.id).catch((err) =>
    console.error('[Worker] 异步生成摘要失败:', err)
  );
  ...
  try {
    if (weclawClient.isConfigured()) {
      // 自动检测回复情感，发送对应GIF表情包
      const stickerRules: [RegExp, string][] = [
      ...
      try {
        ...
        if (fs.existsSync(stickerDir)) {
          const files = fs.readdirSync(stickerDir).filter((f: string) => /\.(gif|png|jpg|jpeg|webp)$/i.test(f));
...
// =============================================================================
const importWorker = new Worker('chat-import', async (job: Job) => {
  ...
  try {
    ...
    if (stickerList && stickerList.length > 0 && userStickerDir) {
      ...
      for (const sf of stickerList) {
        try {
          ...
          if (!fs.existsSync(dest)) { fs.copyFileSync(sf, dest); copied++; }
        } catch {}
      }
    ...
    const senderCounts: Record<string, { total: number; fromUser: number; notUser: number }> = {};

    // 流式解析 + 批量插入 (每500条一批)，避免全量载入内存
    let total = 0;
    total = await parseChatExportStreaming(
      filePath, 'auto', { userNickname },
      async (batch: any[], progress: number) => {
        ...
        for (const msg of batch) {
          ...
          if (!senderCounts[s]) senderCounts[s] = { total: 0, fromUser: 0, notUser: 0 };
          senderCounts[s].total++;
          if (msg.isFromUser) senderCounts[s].fromUser++;
          else senderCounts[s].notUser++;
          values.push(`($${paramIdx},$${paramIdx+1},$${paramIdx+2},$${paramIdx+3},$${paramIdx+4},$${paramIdx+5})`);
          params.push(taskId, userId, s, msg.content || '', msg.timestamp || new Date(), 'text');
          paramIdx += 6;
        }
        await pgPool.query(
          `INSERT INTO imported_messages (task_id, user_id, sender, content, timestamp, msg_type) VALUES ${values.join(',')}`,
          params
        );
        stored += batch.length;
    ...
    for (const [name, c] of Object.entries(senderCounts)) {
      if (c.notUser > aiBest) { aiBest = c.notUser; aiName = name; }
      if (c.fromUser > userBest) { userBest = c.fromUser; userName = name; }
    }
    ...
    await pgPool.query(`UPDATE import_tasks SET status='done', progress=100, message_count=$1, meta=$2::jsonb WHERE id=$3`,
      [stored, JSON.stringify(updMeta), taskId]);
...
const worker = new Worker<ProcessMessageJob>(QUEUE_NAME, processMessage, {
