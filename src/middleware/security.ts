...
export function sanitize<T>(input: T): T {
  if (typeof input === 'string') return xss(input, xssOptions) as unknown as T;
  if (Array.isArray(input)) return input.map((item) => sanitize(item)) as unknown as T;
...
function validateInput(obj: any, path: string = ''): string | null {
  if (obj === null || obj === undefined) return null;

  if (typeof obj === 'string') {
    if (obj.length > 10000) return `${path}: 内容过长`;
    if (/[\x00-\x08\x0B\x0C\x0E-\x1F]/.test(obj)) return `${path}: 含非法字符`;
  }

  if (Array.isArray(obj)) {
    if (obj.length > 100) return `${path}: 数组过长`;
  ...
  if (obj !== null && typeof obj === 'object' && !Array.isArray(obj)) {
    ...
    if (keys.length > 50) return `${path}: 字段过多`;
...
export function securityGuard(req: Request, res: Response, next: NextFunction): void {
  ...
  if (err) { res.status(400).json({ error: err, code: 'VALIDATION_ERROR' }); return; }

  next();
}

// 保留旧导出兼容性
export const xssSanitizer = securityGuard;
export const sqlInjectionGuard = securityGuard;
export const inputValidator = securityGuard;
export const securityLogger = securityGuard;
