...
import { Pool } from 'pg';
import { signAccessToken, signRefreshToken, verifyToken, authenticate, AuthUser } from '../middleware/auth';
import { generateCaptcha, verifyCaptcha } from '../middleware/captcha';
...
export function initAuthRoutes(pool: Pool): void {
  pgPool = pool;
}
...
/** 登录失败计数器 (IP级别，存储在内存中) */
const loginFailures: Map<string, { count: number; lastAttempt: number }> = new Map();
 ...
 * 检查是否需要验证码
 */
function requireCaptcha(ip: string): boolean {
...
// GET /api/auth/captcha — 获取图形验证码
// =============================================================================
router.get('/captcha', captchaLimiter, async (_req: Request, res: Response) => {
  try {
...
router.post('/register', strictLimiter, async (req: Request, res: Response) => {
  try {
    ...
    const invResult = await pgPool.query(
      'SELECT * FROM invite_codes WHERE code=$1 AND is_active=TRUE AND (expires_at IS NULL OR expires_at > NOW()) AND use_count < max_uses',
      [inviteCode]
    );
    ...
    // 检查 wechat_id 是否冲突
    if (wechatId) {
      const wechatExist = await pgPool.query(
        'SELECT id FROM user_accounts WHERE wechat_id = $1',
        [wechatId]
      );
    ...
    const result = await pgPool.query(
      `INSERT INTO user_accounts (email, password_hash, display_name, role, wechat_id)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id, email, display_name, role, wechat_id, created_at`,
      [email.toLowerCase().trim(), passwordHash, displayName || email.split('@')[0], role, wechatId || null]
    );
    ...
    const inv = await pgPool.query(
      'UPDATE invite_codes SET use_count = use_count + 1 WHERE code = $1 RETURNING created_by, bonus_credits, invitee_credits',
      [inviteCode]
    );
...
router.post('/login', strictLimiter, async (req: Request, res: Response) => {
  try {
    ...
    // 查找用户
    const result = await pgPool.query(
      `SELECT id, email, password_hash, display_name, role, wechat_id, is_active
    ...
    await pgPool.query(
      ...
      [user.id, refreshToken, req.ip || 'unknown']
    );
...
router.post('/refresh', async (req: Request, res: Response) => {
  try {
    ...
    const user = await pgPool.query(
      'SELECT id, email, role, wechat_id FROM user_accounts WHERE id = $1 AND is_active = TRUE',
      [decoded.userId]
    );
    ...
    await pgPool.query(
      `INSERT INTO user_sessions (user_id, refresh_token, expires_at, ip_address)
       VALUES ($1, $2, NOW() + INTERVAL '7 days', $3)`,
      [u.id, newRefreshToken, req.ip || 'unknown']
    );
...
// =============================================================================
router.post('/link-wechat', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    // 检查是否已被绑定
    const existing = await pgPool.query(
      'SELECT id FROM user_accounts WHERE wechat_id = $1 AND id != $2',
      [wechatId, req.user!.userId]
    );
    if (existing.rows.length > 0) {
      res.status(409).json({ error: '该微信ID已被其他账号绑定' });
      return;
    ...
    await pgPool.query(
      'UPDATE user_accounts SET wechat_id = $1 WHERE id = $2',
      [wechatId, req.user!.userId]
    );
