...
function setupWebSocket() {
    ...
    ws.onopen = () => {
        ...
        // Send identify handshake so the relay knows which bot/character this is
        const ctx = getContext();
        const identifyPayload = {
            type: 'identify',
            bot_id: extension_settings[extensionName].botId || '24926e2e3aa4@im.bot',
            wechat_id: extension_settings[extensionName].wechatId || 'default',
            char_name: extension_settings[extensionName].charName || ctx.name2 || 'Assistant',
        };
        ws.send(JSON.stringify(identifyPayload));
        updateDebugLog(`已发送身份: ${identifyPayload.char_name}`);
    };
...
jQuery(async () => {
    ...
    // === Reply Capture: Send AI-generated replies back through WebSocket ===
    const context = getContext();
    if (context.eventSource && context.eventTypes) {
        context.eventSource.on(context.eventTypes.GENERATION_ENDED, () => {
            if (!ws || ws.readyState !== WebSocket.OPEN) {
                updateDebugLog('WebSocket未连接，跳过回复发送');
                return;
            }
            ...
            try {
                ...
                if (!lastMsg || lastMsg.is_user) {
                    updateDebugLog('最后一条不是AI消息，跳过');
                    return;
                }

                const replyPayload = {
                    type: 'reply',
                    user_id: context.name1 || 'user',
                    content: lastMsg.mes,
                    bot_id: extension_settings[extensionName].botId || '',
                    action: 'send_message',
                };

                ws.send(JSON.stringify(replyPayload));
                updateDebugLog(`回复已发送: ${lastMsg.mes.substring(0, 30)}...`);
            } catch (e) {
                updateDebugLog(`发送回复失败: ${e.message}`);
            }
        });
        updateDebugLog('回复捕获已启用');
    } else {
        updateDebugLog('警告: 无法访问事件系统，回复捕获未启用');
    }
